...
from src.FineTuneZephyr.config.mongoDbConfiguration import ServerConfigurationManager
from src.FineTuneZephyr.utils.common import get_size


class DataIngestion:
    def __init__(self,DataIngestionConfig : DataIngestionConfig) -> None:
    ...
    def Download_Data(self):
        ...
        if not os.path.exists(self.data_ingestion_config.local_file_path):
            ...
            df.to_csv(self.data_ingestion_config.local_file_path,index=False)
            print(df.head())
            logger.info("Data saved successful in form of CSV file")

        else: 
            logger.info(f"File already exists of size: {get_size(Path(self.data_ingestion_config.local_file_path))}")   